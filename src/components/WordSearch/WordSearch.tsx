...
const WordSearch: React.FC<WordSearchProps> = ({ input = '' }) => {
    ...
    const fetchSuggestions = async (word: string) => {
        await customAxios.get(`/dictionaries?word=${word}`)
            .then(res => {
                const suggestions = res.data.words.map((item: any) => item);
            ...
            <form className="flex items-center">
                ...
                <div className="relative w-full">
                    ...
                    {suggestions.length > 0 && isInputFocused && (
                        <div className="suggestions-container z-40 absolute w-full flex flex-col border border-gray-300 bg-white rounded-lg p-3 top-12 cursor-pointer">
                            {suggestions.map((suggestion, index) => (
                                <Link to={`/dictionary/${suggestion?.word}`} key={index} className="z-50 suggestion-item rounded items-center justify-between p-2.5 hover:bg-gray-50 block">
                                    ...
                                    <p className="text-gray-900 text-sm">{suggestion?.means[0]?.meaning}</p>
                                </Link>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="absolute inset-y-0 end-0 flex items-center">
                        <button type="button" className="inset-y-0 end-0 flex items-center pe-3" onClick={() => setOpenModal(!openModal)}>
                            <FontAwesomeIcon icon={faPenToSquare} className="w-4 h-4 text-gray-500" />
                        </button>
                        {openModal && (
