...
function CreateDictionaryPage() {
  ...
  const [wordId, setWordId] = useState(uuidv4());
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    await customAxios
      ...
      .then((res) => {
        if (res.status === 200) {
          Toastify.success("Create word successfully");
          resetFields();
  ...
  return (
    <div className="create_dictionarypage_container overflow-y-scroll bg-[#F6F7FB]">
      <div className="create_dictionary_page_content w-full h-screen pt-24 flex flex-col items-center justify-start ">
